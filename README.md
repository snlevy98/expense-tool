...
| `PLAID_SYNC_START_DATE` | No | Earliest transaction date to ingest from Plaid, ISO format (e.g. `2026-01-01`). Set it to the day after your last CSV import so the Plaid backfill doesn't duplicate history already in the database. Empty = backfill up to 730 days. |
